...
class ArgList():
    ...
    def __init__(self):
        ...
        stmt_dummy = STMTTRN(trntype="OTHER", dtposted=datetime.now(ofxtools.utils.UTC), trnamt=0.01,fitid="x")
...
def cli_arg_constructor() -> ArgumentParser:
    ...
    cmdparser = argparse.ArgumentParser(description="Converts OFX and QFX files to CSV format")
    cmdparser.add_argument('source_file', help='OFX or QFX file to convert to CSV')

    for option in arg_dict:
        if not option == 'source_file':
            cmdparser.add_argument('--'+option, help="Include this data in the CSV", action="store_true")
    return cmdparser
